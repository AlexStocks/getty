...
type RPCClientMap struct {
	...
	dirty map[string]*entryRPCClientMap
...
type readOnlyRPCClientMap struct {
	m       map[string]*entryRPCClientMap
...
func (m *RPCClientMap) Store(key string, value *rpcClientArray) {
	...
	if e, ok := read.m[key]; ok {
		...
		if !read.amended {
			...
			m.dirtyLocked()
			m.read.Store(readOnlyRPCClientMap{m: read.m, amended: true})
		}
		m.dirty[key] = newEntryRPCClientMap(value)
	}
	m.mu.Unlock()
}
...
func (e *entryRPCClientMap) unexpungeLocked() (wasExpunged bool) {
	return atomic.CompareAndSwapPointer(&e.p, expungedRPCClientMap, nil)
}
...
func (e *entryRPCClientMap) delete() (hadValue bool) {
	for {
		p := atomic.LoadPointer(&e.p)
		if p == nil || p == expungedRPCClientMap {
			return false
...
func (m *RPCClientMap) Range(f func(key string, value *rpcClientArray) bool) {
	...
	if read.amended {
		...
		read, _ = m.read.Load().(readOnlyRPCClientMap)
		if read.amended {
			read = readOnlyRPCClientMap{m: m.dirty}
			m.read.Store(read)
			m.dirty = nil
			m.misses = 0
		}
...
func (m *RPCClientMap) missLocked() {
	m.misses++
	if m.misses < len(m.dirty) {
		return
	}
	m.read.Store(readOnlyRPCClientMap{m: m.dirty})
	m.dirty = nil
	m.misses = 0
}

func (m *RPCClientMap) dirtyLocked() {
	if m.dirty != nil {
		return
	}

	read, _ := m.read.Load().(readOnlyRPCClientMap)
	m.dirty = make(map[string]*entryRPCClientMap, len(read.m))
	for k, e := range read.m {
...
func (e *entryRPCClientMap) tryExpungeLocked() (isExpunged bool) {
	p := atomic.LoadPointer(&e.p)
	for p == nil {
		if atomic.CompareAndSwapPointer(&e.p, nil, expungedRPCClientMap) {
			return true
		}
		p = atomic.LoadPointer(&e.p)
	}
	return p == expungedRPCClientMap
}
